...
def getCoord(frame):
    ...
    if ids is not None:
        ...
        for i in range(rvec.shape[0]):
            ...
            temp.append(tvec[i, 0, 0]*500)  # x    (mm)
            temp.append(tvec[i, 0, 1]*500)  # y    (mm)
            temp.append(tvec[i, 0, 2]*500)  # z    (mm)
            ...
            wb = openpyxl.load_workbook('route.xlsx', data_only=True)
            s1 = wb['工作表1']
            #s1.delete_rows(1,s1.max_row)
            for i in coord:
                s1.append(i)
            wb.save('route.xlsx')
        return coord
